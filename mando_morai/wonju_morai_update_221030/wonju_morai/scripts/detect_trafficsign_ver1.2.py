...
class IMGParser():
    def __init__(self):
        ...
        self.traffic_signal_pub = rospy.Publisher('/traffic_signal', Int32, queue_size=1)
        # tmp for color bound select
        self.crop_pts = np.array(
                                    [[
                                        [int(x * 0.47), int(y * 0.48)], # left lower
                                        [int(x * 0.47), int(y * 0.42)], # left upper
                                        [int(x * 0.62), int(y * 0.42)], # right upper
                                        [int(x * 0.62), int(y * 0.48)]  # right lower
                                    ]]
                                )
    ...
    def callback(self,msg):
        ...
        traffic_signal = np.argmax(detect_list)

        if traffic_signal == 0:
            print 'red'
        elif traffic_signal == 1:
            print 'yellow'
        else:
            print 'green'
        self.traffic_signal_pub.publish(traffic_signal)
