def select_mission(WP_index, switch_WP):
    ...
    elif (switch_WP==1 and WP_index>=36 and WP_index<=39) or (switch_WP==2 and WP_index>=17 and WP_index<=22) or (switch_WP==3 and WP_index>=1 and WP_index<=2): # stop line, distance
        mission=5

    # No GPS
    elif switch_WP==2 and WP_index>=4 and WP_index<=12:
        mission=6

    # before recognition pedestrian dcc
    elif (switch_WP==1 and WP_index==24) or (switch_WP==2 and WP_index==23) or (switch_WP==1 and WP_index==40):
        mission=77 # pre

    # recognition pedestrian
    elif (switch_WP==1 and WP_index>=25 and WP_index<=28) or (switch_WP==2 and WP_index>=24 and WP_index<=27) or (switch_WP==1 and WP_index>=41 and WP_index<=43):
        mission=7
