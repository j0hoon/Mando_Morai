...
from utils.trajectory_make_by_LD import trajectory_make_LD
from nav_msgs.msg import Odometry
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if mission == 6:
                ...
                if self.min_index_for_LD>=20 or self.init_signal==1:
                    WP_x,WP_y = trajectory_make_LD(x_ego,y_ego,self.des_steer) # problem
                    self.init_signal = 0

                self.ctrl_msg.steering,self.min_index_for_LD = pure_pursuit(WP_x, WP_y, target_velocity, x_ego,y_ego,ego_head) # ego_status_vel_x, ego_status_vel_y
                
                self.ctrl_msg.steering = self.des_steer ############# delete!! #############

                print 'min index : ',self.min_index_for_LD
