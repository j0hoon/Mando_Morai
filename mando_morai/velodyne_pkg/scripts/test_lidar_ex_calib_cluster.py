...
class LiDARToCameraTransform:
    def __init__(self, params_cam, params_lidar):
        ...
        # self.track_sub = rospy.Subscriber("/track", Float64MulitArray, self.track_callback)
        self.image_sub = rospy.Subscriber("/image_jpeg/compressed", CompressedImage, self.img_callback)
        self.track_sub=rospy.Subscriber("/track",Float64MultiArray,self.callback)
        self.pc_np = None
        self.img = None
        ...
        while not rospy.is_shutdown():
            if self.track_message:
                ...
                rospy.loginfo("Waiting for track data")
    ...
    def scan_callback(self, msg):
        ...
        for point in pc2.read_points(msg, skip_nans=True):
            point_list.append((point[0], point[1], point[2], 1)) # self.pc_np = x,y,z
        self.pc_np = np.array(point_list, np.float32)

    def callback(self,msg):
        self.track_message=msg # [] MultiArray

    def sub_track(self,msg):
        track_data=msg.data
        return track_data
...
def draw_pts_img(img, xi, yi):
    ...
    for ctr in zip(xi, yi):
        point_np = cv2.circle(point_np, ctr, 5, (255,0,0),-1) # init radius = 2
    return point_np

def track_manage(track_data): # track length = 5
    track=[]
    for i in range(len(track_data)//7):
        x_esti=track_data[i*7]
        x_dot_esti=track_data[i*7+1]
        y_esti=track_data[i*7+2]
        y_dot_esti=track_data[i*7+3]
        local_x,local_y,z=track_data[i*7+4],track_data[i*7+5],track_data[i*7+6]
        track.append([x_esti,x_dot_esti,y_esti,y_dot_esti,local_x,local_y,z])
    return track
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        track_data = Transformer.sub_track(Transformer.track_message)
        managed_track=track_manage(track_data)
        print("track data : ",len(managed_track))
        managed_track=np.array(managed_track) # managed track = track data storage = [x_esti, x_dot_esti, y_esti, y_dot_esti, z]

        #print('track shape :' ,managed_track.shape)
        xyz_p = Transformer.pc_np[:, 0:3]
        if len(managed_track)!=0:
            xyz_p_track = managed_track[:,[4,5,6]]
            #print('xyz_p : ',xyz_p)
            #print('xyz_p length : ',len(xyz_p)) # 20000
            #print('xyz_p shape : ',xyz_p.shape) # 20000   
            #print("xyz_p_track : ",xyz_p_track)
            xyz_p = np.insert(xyz_p,3,1,axis=1).T
            xyz_p_track = np.insert(xyz_p_track,3,1,axis=1).T
            #print("xyz_p_track 2: ",xyz_p_track)
            #print('xyz_p.T shape : ',xyz_p.shape) # 20000
            #print('xyz_p_track.T shape : ',xyz_p_track.shape) # 20000            
            xyz_p = np.delete(xyz_p,np.where(xyz_p[0,:]<0),axis=1)
            xyz_p_track = np.delete(xyz_p_track,np.where(xyz_p_track[0,:]<0),axis=1)
            #print('xyz_p_track.T shape 2  : ',xyz_p_track.shape) # 20000 
            #print('xyz_p.delete shape : ',xyz_p.shape)
            ...
            xyz_c_track = Transformer.transformLiDARToCamera(xyz_p_track)

            #print('xyz_c shape : ',xyz_c.shape)
            #print('xyz_c_track shape : ',xyz_c_track.shape)            
            #print(np.size(xyz_c[0]))

            xy_i = Transformer.transformCameraToImage(xyz_c)
            xy_i_track = Transformer.transformCameraToImage(xyz_c_track)
            #print('xy_i : ',xy_i)
            #print('xy_i shape : ',xy_i.shape)   
            #print('xy_track i shape : ',xy_i_track.shape)   # why 3,0????
                 
            #print(np.size(xy_i[0]))

            #TODO: (6) PointCloud가 Image에 투영된 Processed Image 시각화
            xy_i = xy_i.astype(np.int32)
            xy_i_track = xy_i_track.astype(np.int32)
            #projectionImage = draw_pts_img(Transformer.img, xy_i[0,:], xy_i[1,:])
            projectionImage = draw_pts_img(Transformer.img, xy_i_track[0,:], xy_i_track[1,:])
