...
from maneuver_planner_msg.msg import maneuver_planner_msg, LC_complete_msg, Risk, Object_state
...
lane_curvature_for_curve_2 = 0.15 # 0.04
...
class ManeuverPlanner():
    def __init__(self):
        ...
        # Publish
        self.mode_pub = rospy.Publisher('/mode', maneuver_planner_msg , queue_size=3) # init = 3
        self.risk_pub = rospy.Publisher('/risk', Risk , queue_size=3)
        self.object_state_pub = rospy.Publisher('/object_state', Object_state , queue_size=3)
        # subscriber
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                mission_number = 0
            # if self.rotary_go == 1:
            #     print("rel y : ",object_status_rel_y_1) # print("mission :",mission_number )
            ...
            if mission_number == 0 :
                ...
                risk, object_state = -1, 2 
                self.risk_pub.publish(risk)
                self.object_state_pub.publish(object_state)
                rate.sleep()
                ...
                if (ego_heading <= 174 and ego_heading >= 120) or (ego_heading <= -120 and ego_heading >= -174): # abs(ego_heading - old_heading) > 5:
                    ...
                    time.sleep(0.12) # time.sleep(0.1)  
                # print("maneuver_mode : ", self.maneuver_mode,'object state : ',object_state) # "maneuver_mode : ", self.maneuver_mode,'object state : ',object_state
                ...
                self.maneuver_mode                                   = self.ManeuverMode(risk, object_state, trafficlight_signal)
                if object_status_rel_y_1 < 0: # abs(ego_heading - old_heading) > 5:
                    self.maneuver_mode = 8
                    time.sleep(0.1)
            ...
            elif mission_number == 55: # for mission 4 at curve
                if object_status_rel_x_1 > 0:
                    object_status_x = object_status_x_1
                    object_status_y = object_status_y_1
                    object_status_d = object_status_d_1
                ...
                risk,IVT, MSD, RISK_F                           = self.threatAssessment(D_nearest, ego_status_velocity)
                object_state                                    = self.RecognitionObject(lane_offset, lane_curvature_for_curve_2, object_status_rel_x, object_status_rel_y)
                self.maneuver_mode                                   = self.ManeuverMode(risk, object_state, trafficlight_signal)
                if object_status_rel_y_1 < 0: # abs(ego_heading - old_heading) > 5:
                    self.maneuver_mode = 8
                    time.sleep(0.1)
                ...
                self.risk_pub.publish(risk)
                self.object_state_pub.publish(object_state)
                # print('object state distance : ',object_status_d)
                ...
                if (k_1_object_status_d < k_object_status_d) and k_1_object_status_d != 0 and object_status_rel_y_1 < 0:
                    self.maneuver_mode = 7
                    self.rotary_go = 0
                else:
                    if self.rotary_go == 1:
                ...
                k_1_object_status_d = k_object_status_d
